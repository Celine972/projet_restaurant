...
$search = [];
	...
	$select = $bdd->prepare('SELECT * FROM recipe INNER JOIN users ON recipe.id_author = users.id_user WHERE recipe.id_author = users.id_user LIKE '%post['search']%'');
	if($select->execute()){
		echo 'toto';
...
<h1>Afficher les recherche</h1>

<p><?php //echo $search['search']?></p>
